...
$sql = "SELECT * FROM transaction_sapling";
...
$sql = "SELECT * FROM transaction_seed";

$result = $conn-> query($sql);

if($result->num_rows > 0){
    while($row = $result-> fetch_assoc()){
        echo "<tr><td>". $row["transaction_id_seed"]. "</td><td>". $row["shop_id"]. "</td><td>". $row["seed_id"]. "</td><td>". $row["customer_id"]. "</td><td>". $row["date_seed"]. "</td><td>". $row["stock_change_seed"]. "</td><td>". $row["sale_total_seed"]. "</td></tr>";       
}
}
else{
    echo "No Result";
